...
if(isset($_POST['addincentive']))
{
	...
	// $sql=$pdo->query("SELECT * FROM `incentive` where  name LIKE '$name'");
	// $num=$sql->rowCount();
	...
	{	
		...
		$Filename=date('dmyhis').basename( $_FILES['fld_image']['name']);		
			$posted_data['fld_image']=$Filename;	
				...
				$affected_rows=insert('incentive',$posted_data);
...
if(isset($_POST['deleteall']))
{
	...
	$data=sqlfetch("select * from `incentive` where id in ($str_rest_refs)");
	...
	$pdo=getPDOObject();
	$q=$pdo->query("DELETE FROM `incentive` WHERE id in ($str_rest_refs)");
	if($q)
	$umessage='<div class="alert alert-success" role="alert">
...
if(isset($_POST['editdone']))
{
	...
	{	$Filename='';
	 ...
	 $affected_rows=update('incentive',$posted_data,array('id'=>$pid));
...
function incentive_form($pid='0',$admin_id=0,$amt='0.00',$formname='addincentive')
{ ?>
                        ...
                        <div class="col-md-12">
                            <div class="form-group">
                                <div class="col-md-offset-2 col-lg-10">
                                    ...
                                    <button name="<?=$formname; ?>" class="btn btn-primary " type="submit" id="submit"><i class="fa fa-check"></i> Save</button> | <a href="<?=SITE_URL;?>view_incentive.php">Or Cancel</a>
                                </div>
                            </div>
                        </div>
                    </div>
      ...
      <div class="main_container">
        ...
        <div class="right_col" role="main">
            <div class="row wrapper white-bg page-heading">
                <div class="col-lg-12">
                    <h2 style="color: #2F4050; font-size: 16px; font-weight: 400; margin-top: 18px">incentive</h2>
                </div>
            </div>
...
{ ?>
        <div class="ibox float-e-margins">
            <div class="ibox-title">
                <h5>Edit incentive</h5>
            </div>
            <div class="ibox-content" id="ibox_form">
                <?=$umessage;?>
			<?php 
			$pid=$_GET['pid'];
			$incentive_data=sqlfetch("SELECT * FROM `incentive` where id='$pid' ");
			foreach($incentive_data as $incentive)
			{
				extract($incentive);
                incentive_form($pid,$name,$services_id,$max_no_of_product,$max_no_of_enquiry,$des,$actstat,$fld_order,$price,$formname='editdone');
			}
			?>
			</div>
        </div>
