...
$total_verified_by_data=sqlfetch("SELECT id,cid from `new_data_list` where verified_by='get_admin_id()'");
$total_inserted_count=count($total_verified_by_data);
$total_verified_by_data_today=sqlfetch("SELECT * FROM `new_data_list` where admin_id='$admin_id' and dated LIKE 
'$date_today%'");
$total_inserted_count_today=count($total_verified_by_data_today);
$customer_id_arr_total=array();
// print_r($total_verified_by_data);

if(count($total_verified_by_data))
	foreach($total_verified_by_data as $total_inserted)
...
if(count($total_verified_by_data_today))
	foreach($total_verified_by_data_today as $total_inserted_today)
...
<!-- top tiles -->
          <div class="row tile_count">
			...
			<div class="col-md-2 col-sm-4 col-xs-6 tile_stats_count">
              <span class="count_top"><i class="fa fa-user"></i>Today Verified Data Data</span>
	...
	else
	{
		...
		$data_entered=sqlfetch("SELECT * FROM new_data_list where verified_by='$admin_id' and dated Like ''");
