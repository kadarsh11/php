...
if(isset($_POST['addsale_target']))
{
	...
	$sql=$pdo->query("SELECT * FROM `sale_target` where  sales_id LIKE '$sales_id'");
	$num=$sql->rowCount();
	...
	{	
		...
		$Filename=date('dmyhis').basename( $_FILES['fld_image']['name']);		
			$posted_data['fld_image']=$Filename;	
				...
				$affected_rows=insert('sale_target',$posted_data);
...
if(isset($_POST['deleteall']))
{
	...
	$data=sqlfetch("select * from `sale_target` where id in ($str_rest_refs)");
	...
	$pdo=getPDOObject();
	$q=$pdo->query("DELETE FROM `sale_target` WHERE id in ($str_rest_refs)");
	if($q)
	$umessage='<div class="alert alert-success" role="alert">
...
if(isset($_POST['activate']))
{
	...
	if(count($arr))
	{
		$str_rest_refs=implode(",",$arr);
		$pdo=getPDOObject();
	$q=$pdo->query("UPDATE `sale_target` SET actstat='1' WHERE id in ($str_rest_refs)");
	if($q)
	$umessage='<div class="alert alert-success" role="alert">
...
if(isset($_POST['deactivate']))
{
	...
	if(count($arr))
	{
		$str_rest_refs=implode(",",$arr);
		$pdo=getPDOObject();
	$q=$pdo->query("UPDATE `sale_target` SET actstat='0' WHERE id in ($str_rest_refs)");
	if($q)
	$umessage='<div class="alert alert-success" role="alert">
...
if(isset($_POST['editdone']))
{
	...
	{	$Filename='';
	 ...
	 $affected_rows=update('sale_target',$posted_data,array('id'=>$pid));
...
function sale_target_form($pid='0',$sales_id='0',$target='0.00',$formname='addsale_target')
{ ?>
                        ...
                        <div class="col-md-11 col-sm-12 col-md-pull-1">
							<div class="form-group">
								...
								<div class="col-md-8 col-sm-8 col-xs-12">
								  <select class="select2_multiple form-control" name="sales_id" multiple="multiple">
									<?php 
									$admin_id_data=sqlfetch("select * from `admin` where actstat='1' and admin_role_id='2' ");
								if(count($admin_id_data))
									foreach($admin_id_data as $admin_id)
									{
										$select='';
									if($admin_id['id']==$sales_id)
                        ...
                        <div class="col-md-12">
                            <div class="form-group">
                                <div class="col-md-offset-2 col-lg-10">
                                    ...
                                    <button name="<?=$formname; ?>" class="btn btn-primary " type="submit" id="submit"><i class="fa fa-check"></i> Save</button> | <a href="<?=SITE_URL;?>view_sale_target.php">Or Cancel</a>
                                </div>
                            </div>
                        </div>
                    </div>
      ...
      <div class="main_container">
        ...
        <div class="right_col" role="main">
            <div class="row wrapper white-bg page-heading">
                <div class="col-lg-12">
                    <h2 style="color: #2F4050; font-size: 16px; font-weight: 400; margin-top: 18px">sale_target</h2>
                </div>
            </div>
...
{ ?>
        <div class="ibox float-e-margins">
            <div class="ibox-title">
                <h5>Edit sale_target</h5>
            </div>
            <div class="ibox-content" id="ibox_form">
                <?=$umessage;?>
			<?php 
			$pid=$_GET['pid'];
			$sale_target_data=sqlfetch("SELECT * FROM `sale_target` where id='$pid' ");
			foreach($sale_target_data as $sale_target)
			{
				extract($sale_target);
                sale_target_form($pid,$sales_id,$target,$formname='editdone');
			}
			?>
			</div>
        </div>
<?php 
	...
	?>
        <div class="ibox float-e-margins">
            <div class="ibox-title">
                <h5>Add sale_target</h5>
            </div>
            <div class="ibox-content" id="ibox_form">
                <?=$umessage;?>

                <?php sale_target_form();?>
			</div>
        </div>
