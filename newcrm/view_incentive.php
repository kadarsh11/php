...
if(isset($_POST['activate']))
{
	...
	if(count($arr))
	{
		$str_rest_refs=implode(",",$arr);
		$pdo=getPDOObject();
	$q=$pdo->query("UPDATE `incentive` SET actstat='1' WHERE id in ($str_rest_refs)");
	if($q)
	$umessage='<div class="alert alert-success" role="alert">
...
if(isset($_POST['deactivate']))
{
	...
	if(count($arr))
	{
		$str_rest_refs=implode(",",$arr);
		$pdo=getPDOObject();
	$q=$pdo->query("UPDATE `incentive` SET actstat='0' WHERE id in ($str_rest_refs)");
	if($q)
	$umessage='<div class="alert alert-success" role="alert">
					...
					<div class="x_panel">
					  ...
					  <div class="x_content">
						...
						<table id="datatable-checkbox" class="table table-striped table-bordered bulk_action">
						  ...
						  <tbody>
							<?php 
							$incentive_data=sqlfetch("SELECT * FROM incentive order by id desc");
							if(count($incentive_data))
								foreach($incentive_data as $incentive)
								{ ?>
									

							<tr>
							  <td><input type="checkbox" value="<?=$incentive['id'];?>" class="flat" name="table_records[]"></td>
							  <td><?=get_admin_name_by_id($incentive['admin_id']);?></td>
							  <td><?=$incentive['amt'];?></td>
							  <td><?=$incentive['dated'];?></td>
							  <td>
								<a href="add_incentive.php?pid=<?=$incentive['id'];?>&edit=true"><i class="fa fa-pencil pull-left"></i></a>
							  </td>
							</tr>
							<?php 
								}
