...
if(isset($_POST['deleteall']))
{
	...
	$data=sqlfetch("select * from `quote` where id in ($str_rest_refs)");
	...
	$pdo=getPDOObject();
	$q=$pdo->query("DELETE FROM `quote` WHERE id in ($str_rest_refs)");
	if($q)
...
if(isset($_POST['activate']))
{
	...
	if(count($arr))
	{
		$str_rest_refs=implode(",",$arr);
		$pdo=getPDOObject();
	$q=$pdo->query("UPDATE `quote` SET actstat='1' WHERE id in ($str_rest_refs)");
	if($q)
	$umessage='<div class="alert alert-success" role="alert">
...
if(isset($_POST['deactivate']))
{
	...
	if(count($arr))
	{
		$str_rest_refs=implode(",",$arr);
		$pdo=getPDOObject();
	$q=$pdo->query("UPDATE `quote` SET actstat='0' WHERE id in ($str_rest_refs)");
	if($q)
	$umessage='<div class="alert alert-success" role="alert">
...
$inv_no=get_auto_increment('quote');
?>
	...
	<div class="main_container">
				...
				<div class="row">
					<div class="particulars" >
						<table>
							...
							<tr style="background-color:#f4f6f7; text-align:center;">
								<td><?php echo $i+1; ?></td>
								<td width="140"><input type="text" name="service[]"></td>
								<td width="180"><input type="text" name="description[]"></td>
								<td><input type="text" name="tenure[]"></td>
								<td align="right" width="70"><input type="texet" name="rate[]"></td>
								<td align="right" width="70"><input type="texet" name="amount[]"></td>
							</tr>
							<?php } ?>
						</table>
					</div>
		...
		<input type="submit" name="quote_create" class="btn btn-primary pull-right" value="Submit Quote">
